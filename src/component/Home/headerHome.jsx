...
import { Input, Image, Button, Header, Flex } from "@fluentui/react-northstar"
...
const HeaderHome = () => {
    ...
    const statusMenu = useSelector(state => state.appMcs.status)
    return (
        <Header className="flex justify-around pr-12 2xl:pr-0 xl-pr-0 items-center w-[100%] h-[48px] bg-[#E4EDF3] px-2 md:px-14 sm:justify-between fixed z-50">
            <Flex className="w-10 -ml-10 sm:hidden md:block xl:block 2xl:block">
                <Image src="https://spsvn.com/wp-content/uploads/2023/03/logoSPS.png" alt="" className="w-[100%] h-[35px] object-contain hidden md:block xl:block 2xl:block" />
            </Flex>
            <Flex className="w-10 -ml-10 sm:ml-10 sm:block md:hidden xl:hidden 2xl:hidden">
                <Button className="w-[25px] h-[25px]" onClick={() => dispatch(statusApp(!statusMenu))}>
                    <List className="" />
                    {statusMenu && <MenuHome className="sm:block md:block xl:block 2xl:hidden" />}
                </Button>
            </Flex>

            <Flex className="flex justify-center items-center w-auto">
            ...
            <Flex className="flex justify-center items-center w-32">
